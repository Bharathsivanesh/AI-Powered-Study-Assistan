...
const Topbar = ({ onMenuClick }) => {
  ...
  /* 📥 Listen to staff chat list */
  useEffect(() => {
    if (!open || !staffUid) return;

    const unsubscribe = listenStaffChats(staffUid, setStudents);
    return () => unsubscribe();
  }, [open, staffUid]);

  /* 📩 Select student chat */
  const handleStudentClick = (chat) => {
    setSelectedStudent(chat);
    setMessages([]);

    const unsubscribe = listenToChatMessages(chat.chatId, setMessages);
    return () => unsubscribe();
  };
  ...
  return (
    <div className="flex justify-between items-center bg-white shadow px-4 py-3 gap-3">
      ...
      <h2 className="text-xl font-bold italic text-green-500 flex-1 text-center lg:text-left">
        AI Study Assistant
      </h2>
      ...
      <Modal open={open} onClose={handleClose}>
        ...
        >
          ...
          <div className="border-b px-6 py-3 flex justify-between items-center">
            <div className="flex items-center gap-2">
              <ChatBubbleOutline className="text-green-500" />
              <h2 className="font-semibold text-green-600">Doubt Section</h2>
            </div>
            <button
              onClick={handleClose}
              ...
              {students.length === 0 ? (
                ...
                students.map((s) => (
                  <div
                    ...
                    onClick={() => handleStudentClick(s)}
                    className={`px-4 py-3 cursor-pointer hover:bg-green-50 ${
                      selectedStudent?.chatId === s.chatId ? "bg-green-100" : ""
                    }`}
                  >
                    <div className="flex justify-between">
                      ...
                      <span className="text-xs text-gray-400">
                        {s.lastMessageTime?.toDate?.().toLocaleTimeString()}
                      </span>
                    </div>
                    <p className="text-sm text-gray-600 truncate">
                      {s.lastMessage}
                    ...
                    {messages.map((msg) =>
                      msg.senderRole === "staff" ? (
                        ...
                        <div key={msg.id} className="flex">
                          <div className="bg-white px-4 py-2 rounded-xl max-w-[70%] shadow">
                            <p className="text-sm">{msg.text}</p>
                            <span className="text-[10px] text-gray-500 flex justify-end mt-1">
                              {formatTime(msg.timestamp)}
                            </span>
